...
const Waves: FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      ...
      const { waveSpeedX: wsX, waveSpeedY: wsY, waveAmpX: waX, waveAmpY: waY, friction: fr, tension: tn, maxCursorMove: mcm } = configRef.current;
      lines.forEach((pts) => {
        pts.forEach((p) => {
          ...
          p.cursor.vx += (0 - p.cursor.x) * tn;
          p.cursor.vy += (0 - p.cursor.y) * tn;
          p.cursor.vx *= fr;
          p.cursor.vy *= fr;
    ...
    window.addEventListener("touchmove", onTouchMove, { passive: true }); // passive: true for touchmove
